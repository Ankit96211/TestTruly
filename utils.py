...
def clean2_plate(plate):
    ...
    _, thresh = ret, thresh = cv2.threshold(gray_img, 0, 255, cv2.THRESH_OTSU | cv2.THRESH_BINARY) #cv2.threshold(gray_img, 110, 255, cv2.THRESH_BINARY)
    # cv2.imshow("thresh",gray_img)
    # cv2.waitKey(0)
...
def find_contours(dimensions, img) :
    ...
    for cntr in cntrs :
        ...
        # cv2.imshow("cnt",img[intY:intY+intHeight, intX:intX+intWidth])
        # cv2.waitKey(0)
...
def recognize_plate(img, coords):
    ...
    clean_plate, rect = clean2_plate(box)
    # cv2.imshow("cp", clean_plate)
    # cv2.waitKey(0)
    plate_num=""
    if rect:
        ...
        # cv2.imshow("final",clean_plate)
        # cv2.waitKey(0)
        # reader = easyocr.Reader(['en']) # need to run only once to load model into memory
        # text = reader.readtext(clean_plate)
        text = pytesseract.image_to_string(plate_im, lang='eng',config='-c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ --psm 8 --oem 3')
        plate_num=text
    ...
    #         img_ch = Image.frombytes('RGB', char[i].shape[:2], char[i], 'raw', 'BGR', 0, 0)

    #         text = pytesseract.image_to_string(img_ch)
    #         print("ch ",text)
